<form action="<?= base_url('index.php/process/update/query/'). $query->id ?>" method="post">
  ...
  <div class="mb-1">
    ...
    <input type="text" required name="name" class="form-control" id="companyname" placeholder="Example Company LTD" value="<?= isset($query->name) ? $query->name : '' ?>">
  </div>
  ...
  <select class="form-select" name="database_id" aria-label="Default select example">
    ...
    <?php foreach ($db as $database): ?>
      <option value="<?= $database->id ?>" <?= isset($query->database_id) && $query->database_id == $database->id ? 'selected' : '' ?>><?= $database->db_name ?></option>
    <?php endforeach; ?>
  </select>
  ...
  <div class="mb-1">
    ...
    <div class="input-group">
      ...
      <input type="text" value="<?= isset($query->get) ? $query->get : '' ?>" name="get" class="form-control" id="get" placeholder="key1=value&key2=value4" aria-describedby="basic-addon3 basic-addon4">
    </div>
  </div>
  ...
  <div class="row">
    <div class="col-md-8">
      ...
      <div class="position-relative" style="width:100%; height:200px">
        <div id="editor" class="editor"><?= isset($query->code) ? $query->code : '' ?></div>
      </div>
    </div>
    <div class="col-md-4">
  ...
  <textarea name="code" id="query-field" class="d-none"><?= $query->code ?></textarea>
...
<script>
    ...
    $(".testQuery").click(function(){
      ...
      if(db != ""){
        let urls = "<?= base_url() ?>index.php/process/testQuery/" + db  + "?" + get
        $.ajax({
          type : "POST",
          url : urls,
          data : {query : val},
          success : function(res) {
            ...
            if(data.error){
              $(".query-result").html("<em>Syntax Error</em>")
              console.log("error");
              console.log(data);
            } else {
